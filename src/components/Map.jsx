...
const Map = () => {
    ...
    useEffect(() => {
        ...
        if (cursorCircle.current) {
            cursorCircle.current.addTo(initialMap)
        }
    ...
    useEffect(() => {
        ...
        const checkCursorPosition = (e) => {
            if (cursorCircle.current != null ){
                if (loading) {
                    cursorCircle.current.setStyle({
                        color: 'rgba(0, 0, 0, 0)', //transparent circle
                        fillOpacity: 0
                    });
                } else {
                    cursorCircle.current.setStyle({
                ...
                cursorCircle.current.setLatLng(e.latlng);
                cursorCircle.current.setRadius(radiuskm * 1000)

            } else {
                const newCursorCircle = L.circle(e.latlng, {
                    color: '#4b734e',
                    fillColor: '#1c5721',
                    fillOpacity: 0.1,
                    radius: radiuskm * 1000
                }).addTo(map);
                cursorCircle.current = newCursorCircle
            } 
        }
        const setCircleTransparent = () => {
            if (cursorCircle.current) {
                cursorCircle.current.setStyle({
                    color: 'rgba(0, 0, 0, 0)', //transparent circle
                    fillOpacity: 0
        ...
        map.on('mousemove', checkCursorPosition);
        map.on('mouseout', setCircleTransparent);
